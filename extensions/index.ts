...
const MERIDIAN_MODELS: ProviderModelConfig[] = [
	{
		id: "claude-sonnet-5",
...
function getAdaptiveEffort(modelId: string, thinkingLevel: string): string {
	...
	if (thinkingLevel === "xhigh" && MAX_EFFORT_MODEL_IDS.has(modelId)) {
		return "max";
	}
...
function normalizeModelRequest(
	...
	if (SAMPLING_UNSUPPORTED_MODEL_IDS.has(modelId)) {
	...
	const display = normalized.thinking.display;
	normalized.thinking = {
		type: "adaptive",
		...(display === "summarized" || display === "omitted" ? { display } : {}),
	};
	normalized.output_config = {
		...(isRecord(normalized.output_config) ? normalized.output_config : {}),
		effort: getAdaptiveEffort(modelId, thinkingLevel),
	};
	return normalized;
}
...
function getMinimumVersionWarning(version?: string): string | null {
	if (!version || compareSemver(version, MIN_MERIDIAN_VERSION) >= 0) return null;
	return `This extension requires Meridian >=${MIN_MERIDIAN_VERSION}; runtime v${version} may not support the registered models.`;
}
	...
	pi.registerCommand("meridian", {
		...
		handler: async (args, ctx) => {
			...
			if (subcmd === "version" || subcmd === "update") {
				...
				const lines: string[] = [];
				const runtimeWarning = getMinimumVersionWarning(health.version);
				if (health.version) {
